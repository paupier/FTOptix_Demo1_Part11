...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            ...
            if (editable.Value)
            {
                var groupCheckBox = InformationModel.MakeObject<Panel>(group.BrowseName, FTOptix_Demo1_Part11.ObjectTypes.GroupCheckbox);
            ...
            {
                var groupLabel = InformationModel.MakeObject<Panel>(group.BrowseName, FTOptix_Demo1_Part11.ObjectTypes.GroupLabel);
